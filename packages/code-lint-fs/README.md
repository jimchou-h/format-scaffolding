...
`code-lint-fs` 是[前端编码规范工程化](https://github.com/jimchou-h/format-scaffolding/)的配套 Lint 工具，可以为项目一键接入规范、一键扫描和修复规范问题，保障项目的编码规范和代码质量。
...
| 规范                                                              | Lint 工具                                                  | npm 包                                                                                 |
| ----------------------------------------------------------------- | ---------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| JavaScript 编码规范 <br/> TypeScript 编码规范 <br/> Node 编码规范 | [ESLint](https://eslint.org/)                              | [eslint-config-format-scaffolding](https://www.npmjs.com/package/eslint-config-format-scaffolding)             |
| CSS 编码规范                                                      | [stylelint](https://stylelint.io/)                         | [stylelint-config-format-scaffolding](https://www.npmjs.com/package/stylelint-config-format-scaffolding) |
| Git 规范                                                          | [commitlint](https://commitlint.js.org/#/)                 | [commitlint-config-format-scaffolding](https://www.npmjs.com/package/commitlint-config-format-scaffolding)     |
| 文档规范                                                          | [markdownlint](https://github.com/DavidAnson/markdownlint) | [markdownlint-config-format-scaffolding](https://www.npmjs.com/package/markdownlint-config-format-scaffolding) |

可以看到这些 `Linter` 和规则包众多且零散，全部安装它们会给项目增加十几个依赖，接入和升级成本都比较高。

`code-lint-fs` 收敛屏蔽了这些依赖和配置细节，提供简单的 CLI 和 Node.js API，让项目能够一键接入、一键扫描、一键修复、一键升级，并为项目配置 git commit 卡口，降低项目接入规范的成本。
...
```bash
npm install code-lint-fs -g
```

安装完成后，可执行 `code-lint-fs -h` 以验证安装成功。

### 使用

#### `code-lint-fs init`：一键接入

在项目根目录执行 `code-lint-fs init`，即可一键接入规范，为项目安装规范 `Lint` 所需的依赖和配置。
...
>
> 注 2：如果项目的 .vscode/ 目录被 .gitignore 忽略，可以在拉取项目后单独执行 `code-lint-fs init --vscode` 命令写入 `.vscode/extensions.json` 和 `.vscode/settings.json` 配置文件

#### `code-lint-fs scan`：一键扫描
...
#### `code-lint-fs commit-file-scan` 提交文件扫描
...
#### `code-lint-fs commit-msg-scan` 提交信息扫描
...
- code-lint-fs.init(config)：将项目一键接入规范，效果等同于 `code-lint-fs init`

示例：

```js
(await format-scaffolding) -
  fe -
  lint.init({
    eslintType: 'react',
    enableESLint: true,
...
| ------------------ | ---------- | ------ | ------------------------------------------------------------------------------------------------------------------- |
| cwd                | string     | -      | 项目绝对路径                                                                                                        |
| eslintType         | ESLintType | -      | 语言和框架类型，如果不配置，等同于 code-lint-fs init，控制台会出现选择器，如果配置，控制台就不会出现选择器        |
| enableESLint       | boolean    | true   | 是否启用 ESLint，如果不配置默认值为 true，即默认启用 ESLint                                                         |
| enableStylelint    | boolean    | -      | 是否启用 stylelint，如果不配置，等同于 code-lint-fs init，控制台会出现选择器，如果配置，控制台就不会出现选择器    |
| enableMarkdownlint | boolean    | -      | 是否启用 markdownlint，如果不配置，等同于 code-lint-fs init，控制台会出现选择器，如果配置，控制台就不会出现选择器 |
...
`code-lint-fs` 基于一份配置进行扫描（但你也可以零配置使用），支持的配置参数有：
